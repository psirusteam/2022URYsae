...
$$
Y_d \sim Multinon\left( \theta_{1d}^{N}, \theta_{2d}^{N},\theta_{3d}^{N} \right)
$$

con $d = 1,\cdots, D$ representa los dominios de interés. Los parámetros de interés $\theta_{1d}^{N}, \theta_{2d}^{N}$ y $\theta_{3d}^{N}$ representan la proporción de personas en algún estado laboral (Ocupado, Desocupado o Inactivo). Las estimaciones directas son representadas por $\hat{\theta}_{1d}, \hat{\theta}_{2d}$ y $\hat{\theta}_{3d}$ respectivamente.

## Modelo bayesiano

Para definir el modelo bayesiando debemos considerar una función de enlace para $\boldsymbol{\theta_d} = \left( \theta_{1d}^{N}, \theta_{2d}^{N},\theta_{3d}^{N} \right)$, teniendo presente que la distribución multinomial es la generalización de la distribución binomial la función de enlace natural seria la transformación **logit**.
...
$$
\begin{eqnarray*}
\mu_{1d}&=&\boldsymbol{X}_d^{T}\boldsymbol{\beta_1}+u_{d}\\
\mu_{2d}&=&\boldsymbol{X}_d^{T}\boldsymbol{\beta_2}+u_{d}
\end{eqnarray*}
$$
...
-   *Indicadores_encuesta*: Realiza la estimación de las tasa de interés, de igual forma se tiene la función de *Indicadores_censo* para el calculo de los indicadores en el censo. Los indicadores son calculados son:
...
En interés se centra en la estimación de la Tasa de ocupados, Tasa de desocupados y Tasa de Participación. La variable respuesta es la condición de actividad económica, la cual se identifica en la base de datos como **empleo**. La variable puede tomar los valores
...
En censo fue estandarizado previamente, por tanto, debemos leer el censo y filtrar a los niños de la base de datos.
...
```{r}
byAgrega <- c("depto",  "mpio",   "segm"  , "empleo", "area",   "sexo",   "anoest", "edad", "etnia")
...
-   $Z$ Matriz con los efectos aleatorios en el censo:
...
Después de obtener el resultado, se exportan los resultados con la sintaxis
