...
Estimacion_dir <- diseno %>% group_by(mpio) %>%
  summarise(nd = unweighted(n()),
            thetahat = survey_mean(pobreza, vartype = c("se"),
                                   deff = TRUE)) %>%
...
El modelo propuesto se escribe en `STAN` de la siguiente forma.
...
Xdat <-
  model.matrix(
    thetahat ~ mpio + tasa_desocupacion,
    data = data_dir
  )
