...
readRDS("Data/tasa_desocupacion.rds") %>% 
  rename(Suelo_cultivo = X2016_crops.coverfraction,
         Suelo_urbano = X2016_urban.coverfraction,
         Luces_nocturna = F182013_stable_lights) %>% 
saveRDS("Data/tasa_desocupacion2.rds")  
...
\end{align*}
$$ Para hallar la forma de estimar los coeficientes $\beta$, $\gamma_0$ y $\gamma_1$, Nótese que $y_{ij} = \gamma_0 + \gamma_1 U_{j} + \beta X_{ij} + \epsilon_{ij}+e_{j}$, que a la vez es equivalente a
